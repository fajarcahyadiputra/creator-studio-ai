...
4. `EXTRACTING_AUDIO`
5. `TRANSCRIBING`
6. `DETECTING_SCENES_AND_SILENCE`
7. `ANALYZING_CLIP_CANDIDATES`
8. `RANKING_AND_DEDUPLICATING`
9. `PREPARING_REVIEW_DATA`
10. `COMPLETED_ANALYSIS`
11. `RENDERING_FINAL_CLIPS`
12. `GENERATING_SUBTITLES`
13. `QUALITY_CHECK`
14. `UPLOADING_OUTPUTS`
15. `COMPLETED`

Catatan praktis:

- Kandidat terpilih boleh langsung memicu render workflow tanpa langkah queue manual tambahan.
- Preview artifact MP4 terpisah bersifat opsional bila final output sudah cukup untuk playback/review.
